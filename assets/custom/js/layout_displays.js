		...
		$("#drag_puzzle").empty();
			...
			if(bookmark.current.question_id == key) {
				...
				$.each($("#drag_puzzle li"), function(index, item){
					...
					console.log(current_data.questions[bookmark.current.question_id - 1].drags[id].answer-1);	
...
function bootstrap_checkboxes_slide(slide, callback, in_assess) {
	...
	$.each(slide.answers, function(index, item) {
		...
		item.width_container = Math.floor((100/Number(slide.columns))-(Number(slide.layout[1])+Number(slide.layout[3])+4))+"%";
